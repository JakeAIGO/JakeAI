...
q('#status').onclick=async()=>{try{const rr=await fetch('/v1/commerce/wallet-preview/public-status',{cache:'no-store'});const j=await rr.json();if(!rr.ok)throw new Error(j.detail||('HTTP '+rr.status));out.textContent=JSON.stringify(j,null,2);grid.innerHTML=['payments_enabled','legal_approved','merchant_configured','rpc_configured','admin_review_configured','canary_enabled','preview_only_lock','auto_fulfill_enabled','activation_ready'].map(k=>'<div class="stat"><b>'+k+'</b><br><span class="'+(j[k]?'ok':'off')+'">'+j[k]+'</span></div>').join('');create.disabled=!(j.activation_ready&&j.canary_enabled&&q('#ack').checked&&key.value)}catch(e){out.textContent=e.message;create.disabled=true}}
window.addEventListener('load',()=>q('#status').click());
q('#ack').onchange=()=>q('#status').click();
