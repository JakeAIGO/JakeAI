...
q('#verify').onclick=async()=>{try{const endpoint='__VERIFY_ENDPOINT__';const j=await api(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({order_id:q('#order').value,tx_hash:q('#tx').value})});q('#verifyOut').textContent=JSON.stringify(j,null,2)}catch(e){q('#verifyOut').textContent=e.message}}
    ...
    active_network = (
        "Network: Base Sepolia · Chain ID 84532 · Asset: Circle testnet USDC · Minimum confirmations: 2"
        if testnet_invoice
        else "Network: Base Mainnet · Chain ID 8453 · Asset: native USDC · Minimum confirmations: 2"
    )
    faucets = (
        '<p class="tiny"><b>Free test tokens:</b></p>'
        '<p><a href="https://faucet.circle.com/" target="_blank" rel="noopener">Get free test USDC from Circle →</a></p>'
        '<p><a href="https://www.coinbase.com/developer-platform/products/faucet" target="_blank" rel="noopener">Get free Base Sepolia ETH / USDC from Coinbase Developer Platform →</a></p>'
        '<p class="tiny">These are testnet tokens only and have no monetary value.</p>'
        if testnet_invoice else ""
    )
    html = (
        ...
        .replace("__ACTIVE_NETWORK__", active_network)
        .replace("__FAUCETS__", faucets)
    )
