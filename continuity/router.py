...
provider usage limits. It turns JakeAI-owned state into a portable handoff.
"""
...
def render_handoff(state: dict[str, Any]) -> str:
    ...
    sections = [
        ...
        *([f"- {note}" for note in notes] if notes else ["- None."]),
        ...
        "5. Record completed work back into JakeAI-owned state before another model/session switch.",
        "",
    ]
    return "\n".join(sections)


def write_handoff(state: dict[str, Any], output: Path | None = None) -> Path:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    if output is None:
        stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        output = OUT_DIR / f"handoff-{stamp}.md"
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(render_handoff(state), encoding="utf-8")
    return output
...
def main() -> int:
    ...
    output = write_handoff(state, args.output)
    print(output)
    return 0
