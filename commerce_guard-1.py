...
legacy_create_checkout_session = legacy_main.create_checkout_session
...
app = FastAPI(title="JakeAI Commerce Guard", version="1.4.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://jakeaiofficial.com", "https://www.jakeaiofficial.com"],
    allow_credentials=False,
    allow_methods=["GET", "POST", "OPTIONS"],
    allow_headers=["Content-Type", "Idempotency-Key", "X-Genesis-Admin-Token"],
)

@app.get("/health")
def health():
    return {"status":"healthy","service":"JakeAI Commerce Guard","commerce_mode":"fail_closed","version":"1.4.0","genesis_review_proxy":True}
...
def _legacy_review_handler(name: str):
...
def genesis_review_pending_proxy(x_genesis_admin_token: Optional[str] = Header(None, alias="X-Genesis-Admin-Token")):
    return _legacy_review_handler("genesis_001_pending_review")(x_genesis_admin_token)

@app.post("/v1/genesis/001/review/accept")
@app.post("/api/v1/genesis/001/review/accept")
def genesis_review_accept_proxy(req: GenesisReviewProxyRequest, x_genesis_admin_token: Optional[str] = Header(None, alias="X-Genesis-Admin-Token")):
    return _legacy_review_handler("genesis_001_accept")(req, x_genesis_admin_token)

@app.post("/v1/genesis/001/review/decline-refund")
@app.post("/api/v1/genesis/001/review/decline-refund")
def genesis_review_decline_refund_proxy(req: GenesisReviewProxyRequest, x_genesis_admin_token: Optional[str] = Header(None, alias="X-Genesis-Admin-Token")):
    return _legacy_review_handler("genesis_001_decline_refund")(req, x_genesis_admin_token)

@app.get("/v1/genesis/001/review/health")
@app.get("/api/v1/genesis/001/review/health")
def genesis_review_health_proxy():
    names=("genesis_001_pending_review","genesis_001_accept","genesis_001_decline_refund")
    return {"status":"ready" if all(callable(getattr(legacy_main,n,None)) for n in names) else "incomplete",
            "guard_version":"1.4.0",
            "handlers":{n: callable(getattr(legacy_main,n,None)) for n in names}}
