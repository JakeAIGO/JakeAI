...
def main() -> int:
    ...
    replacements = [
        ...
        ("main.py", '    system_cost: float = Field(..., example=500000.0, description="Gross Turnkey EPC Cost in USD")', '    system_cost: float = Field(..., gt=0.0, example=500000.0, description="Gross Turnkey EPC Cost in USD")'),
        ("main.py", '    system_kw_dc: float = Field(..., example=400.0, description="System DC Nameplate Rating in kW")', '    system_kw_dc: float = Field(..., gt=0.0, example=400.0, description="System DC Nameplate Rating in kW")'),
        ("main.py", '    peak_demand_kw: float = Field(..., example=450.0)', '    peak_demand_kw: float = Field(..., ge=0.0, example=450.0)'),
        ("main.py", '    monthly_consumption_kwh: float = Field(..., example=180000.0)', '    monthly_consumption_kwh: float = Field(..., gt=0.0, example=180000.0)'),
        ("main.py", '    amount: float\n    take_rate: Optional[float] = 0.01', '    amount: float = Field(..., gt=0.0)\n    take_rate: Optional[float] = Field(0.01, ge=0.0, le=1.0)'),
        ("main.py", "            mode='payment',\n            success_url=success_url,", "            mode='payment',\n            metadata={'product_id': product_id},\n            success_url=success_url,"),
        (
            "main.py",
        ...
        ("main.py", '    client_ip = request.client.host if request.client else "unknown"\n    ua = request.headers.get("user-agent", "unknown")\n    \n    # Background honeypot logging for market telemetry\n    background_tasks.add_task(log_unmet_query, q, client_ip, ua, len(matches))', '    # Privacy-minimized product-demand telemetry: retain the search term and match count,\n    # but do not persist requester IP addresses or user-agent strings.\n    background_tasks.add_task(log_unmet_query, q, "not_collected", "not_collected", len(matches))'),
        ("privacy.html", '<li><strong>API Usage Telemetry:</strong> Request timestamps, endpoint routes, response latencies, and IP/user-agent headers strictly for rate limiting, DDoS mitigation, and system debugging.</li>', '<li><strong>API Usage Telemetry:</strong> Limited operational metadata may be processed for security, reliability, abuse prevention, and debugging. Product-search demand telemetry may retain the search term and match count for catalog improvement, but the JakeAI application does not persist the requester IP address or user-agent in that search-demand record.</li>'),
        ("privacy.html", '<li><strong>Transactional Data:</strong> Payment details (processed securely via Stripe; JakeAI never stores raw credit card numbers), transaction hashes, and purchased SKU identifiers.</li>', '<li><strong>Transactional Data:</strong> Where commerce is enabled, payment details are processed by the configured payment provider; JakeAI does not intentionally store raw payment-card numbers. Transaction and purchased-SKU records may be retained as needed to operate and reconcile the service.</li>'),
        ("privacy.html", '<p>All network traffic is encrypted via TLS 1.3. API transactions support standard Idempotency-Key headers to eliminate duplicate billing during network retry cycles.</p>', '<p>Public service traffic is intended to use HTTPS encryption in transit. Supported checkout requests may use Idempotency-Key handling to reduce duplicate transaction creation during client retries; callers should not treat idempotency as a substitute for their own transaction reconciliation.</p>'),
        ("main.py", '        "description": "Deterministic physics calculation for 5-fingered, 22-DoF robotic hands. Solves normal force, tendon tension distribution, joint torque limits, compliance margin, and slip risk.",', '        "description": "Unvalidated educational calculation for hypothetical 5-fingered robotic hands. Outputs are advisory only and must not be used as validated physical-control commands without independent engineering verification.",'),
        ("main.py", '        raise HTTPException(status_code=400, detail=f"Extraction failed: {str(e)}")', '        print(f"Extraction provider error: {type(e).__name__}")\n        raise HTTPException(status_code=400, detail="Extraction failed for the requested public resource.")'),
        ("main.py", '        return {"status": "error", "model": "claude-3-5-sonnet-20241022", "error": str(e)}', '        print(f"Anthropic audit provider error: {type(e).__name__}")\n        return {"status": "error", "model": "claude-3-5-sonnet-20241022", "error": "Provider request failed."}'),
        ("main.py", '        return {"status": "error", "model": "sonar-pro", "error": str(e)}', '        print(f"Perplexity audit provider error: {type(e).__name__}")\n        return {"status": "error", "model": "sonar-pro", "error": "Provider request failed."}'),
        ("main.py", '        raise HTTPException(status_code=400, detail=f"Stripe Error: {str(e)}")', '        print(f"Checkout provider error: {type(e).__name__}")\n        raise HTTPException(status_code=400, detail="Checkout provider could not create a session.")'),
        ("main.py", '        raise HTTPException(status_code=400, detail=f"Failed to retrieve checkout session: {str(e)}")', '        print(f"Checkout verification provider error: {type(e).__name__}")\n        raise HTTPException(status_code=400, detail="Checkout session could not be verified.")'),
        ("main.py", 'class GraspResponse(BaseModel):\n    status: str\n    required_normal_force_newtons: float', 'class GraspResponse(BaseModel):\n    status: str\n    safety_classification: str\n    required_normal_force_newtons: float'),
        ("main.py", '        status="optimized",\n        required_normal_force_newtons=round(required_force, 3),', '        status="advisory_only",\n        safety_classification="UNVALIDATED_PHYSICAL_CONTROL_MODEL",\n        required_normal_force_newtons=round(required_force, 3),'),
        ("main.py", 'def health():\n    return {"status": "healthy", "service": "JakeAI Core v2.3"}', 'def health():\n    """Process liveness only; does not assert dependency or commerce readiness."""\n    return {"status": "alive", "service": "JakeAI Core v2.3", "scope": "process_liveness_only"}'),
    ]
