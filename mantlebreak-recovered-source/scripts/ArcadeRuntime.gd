...
func request_start() -> bool:
    if operator_mode or state in ["PLAY", "BOSS", "READY", "INITIALS"] or not can_start_run():
        return false
    if not free_play:
...
func begin_initials() -> void:
    transition_to("INITIALS")
...
func set_operator_mode(enabled: bool) -> void:
    if state in ["PLAY", "BOSS"] and enabled:
        return
