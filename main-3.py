...
def create_checkout_session(product_id: str, idempotency_key: Optional[str] = Header(None, alias="Idempotency-Key")):
    ...
    if product_id == "prod_genesis_commission_001":
        ...
        try:
            ...
            if state == "reserved":
                # A timed reservation is an unpaid checkout. A NULL reserved_until
                # means payment has been verified and #001 is held pending human review.
                if reserved_until is None or reserved_until > now:
                    conn.execute("ROLLBACK")
                    raise HTTPException(status_code=409, detail="Genesis Commission #001 is currently reserved in another checkout or pending review.")
            conn.execute("UPDATE genesis_commission SET state='reserved', reservation_token=?, stripe_session_id=NULL, customer_email=NULL, problem=NULL, desired_outcome=NULL, current_approach=NULL, notes=NULL, reserved_until=?, updated_at=CURRENT_TIMESTAMP WHERE slot=1", (token, now + 1800))
            conn.execute("COMMIT")
        finally:
            conn.close()
        try:
    ...
    if session_id and stored_session == session_id:
        try:
            ...
            if paid and state == "reserved":
                ...
                conn=sqlite3.connect(DB_PATH)
                # Payment converts the 30-minute checkout hold into a durable paid
                # reservation. #001 is NOT permanently claimed until human review accepts it.
                conn.execute("UPDATE genesis_commission SET customer_email=?, reserved_until=NULL, updated_at=CURRENT_TIMESTAMP WHERE slot=1 AND stripe_session_id=?", (email,session_id))
                conn.commit(); conn.close()
        except Exception:
            paid=False
    pending_review = bool(state == 'reserved' and (paid or reserved_until is None))
    return {"slot":"001","state":state,"paid":paid,"pending_review":pending_review,"intake_allowed": bool(session_id and stored_session==session_id and paid and state in ('reserved','claimed','completed'))}
...
def genesis_001_intake(req: GenesisIntake):
    ...
    if not row or row[0] != req.session_id or row[1] not in ('reserved','claimed','completed'):
        conn.close(); raise HTTPException(status_code=409, detail="This checkout does not own Genesis #001")
    conn.execute("UPDATE genesis_commission SET problem=?,desired_outcome=?,current_approach=?,notes=?,reserved_until=NULL,updated_at=CURRENT_TIMESTAMP WHERE slot=1",(req.problem,req.desired_outcome,req.current_approach,req.notes))
    conn.commit(); conn.close()
    return {"ok":True,"commission":"001","state":"reserved","message":"Genesis Commission #001 intake received. The paid reservation is pending JakeAI human review."}

class GenesisReviewRequest(BaseModel):
    session_id: str = Field(min_length=5, max_length=200)

def _require_genesis_admin(value: Optional[str]):
    expected=os.environ.get("GENESIS_ADMIN_TOKEN","").strip()
    if not expected:
        raise HTTPException(status_code=503, detail="Genesis review control is not configured")
    if not value or not hmac.compare_digest(value, expected):
        raise HTTPException(status_code=403, detail="Genesis review authorization failed")

def _verified_genesis_session(session_id: str):
    secret_key=os.environ.get("STRIPE_SECRET_KEY","").strip()
    if not secret_key:
    ...
    if getattr(sess,'payment_status',None) != 'paid' or metadata.get('jakeai_product_id') != 'prod_genesis_commission_001':
        raise HTTPException(status_code=403, detail="A verified Genesis #001 payment is required")
    return sess

@app.get("/v1/genesis/001/review/pending")
def genesis_001_pending_review(x_genesis_admin_token: Optional[str] = Header(None, alias="X-Genesis-Admin-Token")):
    _require_genesis_admin(x_genesis_admin_token)
    conn=sqlite3.connect(DB_PATH)
    row=conn.execute("SELECT state,stripe_session_id,customer_email,problem,desired_outcome,current_approach,notes,reserved_until,updated_at FROM genesis_commission WHERE slot=1").fetchone()
    conn.close()
    if not row:
        return {"slot":"001","state":"available","paid":False,"pending_review":False}
    state,session_id,email,problem,outcome,current,notes,reserved_until,updated_at=row
    paid=False
    if session_id:
        try:
            secret_key=os.environ.get("STRIPE_SECRET_KEY","").strip(); stripe.api_key=secret_key
            ...
            paid=False
    return {
        "slot":"001","state":state,"paid":paid,
        "pending_review":bool(state=="reserved" and paid),
        "session_id":session_id if paid else None,
        "customer_email":email if paid else None,
        "problem":problem if paid else None,
        "desired_outcome":outcome if paid else None,
        "current_approach":current if paid else None,
        "notes":notes if paid else None,
        "reserved_until":reserved_until,"updated_at":updated_at
    }

@app.post("/v1/genesis/001/review/accept")
def genesis_001_accept(req: GenesisReviewRequest, x_genesis_admin_token: Optional[str] = Header(None, alias="X-Genesis-Admin-Token")):
    _require_genesis_admin(x_genesis_admin_token)
    _verified_genesis_session(req.session_id)
    conn=sqlite3.connect(DB_PATH)
    row=conn.execute("SELECT stripe_session_id,state FROM genesis_commission WHERE slot=1").fetchone()
    if not row or row[0] != req.session_id or row[1] != 'reserved':
        conn.close(); raise HTTPException(status_code=409, detail="Genesis #001 is not awaiting acceptance for this checkout")
    conn.execute("UPDATE genesis_commission SET state='claimed', reserved_until=NULL, updated_at=CURRENT_TIMESTAMP WHERE slot=1")
    conn.commit(); conn.close()
    return {"ok":True,"commission":"001","state":"claimed","message":"Genesis Commission #001 accepted and permanently claimed."}

@app.post("/v1/genesis/001/review/decline-refund")
def genesis_001_decline_refund(req: GenesisReviewRequest, x_genesis_admin_token: Optional[str] = Header(None, alias="X-Genesis-Admin-Token")):
    _require_genesis_admin(x_genesis_admin_token)
    sess=_verified_genesis_session(req.session_id)
    conn=sqlite3.connect(DB_PATH)
    row=conn.execute("SELECT stripe_session_id,state FROM genesis_commission WHERE slot=1").fetchone()
    if not row or row[0] != req.session_id or row[1] not in ('reserved','claimed'):
        conn.close(); raise HTTPException(status_code=409, detail="Genesis #001 is not associated with this checkout")
    payment_intent=getattr(sess,'payment_intent',None)
    if not payment_intent:
        conn.close(); raise HTTPException(status_code=409, detail="No refundable payment intent was found")
    try:
        stripe.Refund.create(payment_intent=payment_intent, reason='requested_by_customer')
    except Exception as exc:
        conn.close(); raise HTTPException(status_code=502, detail=f"Refund could not be confirmed: {exc}")
    conn.execute("UPDATE genesis_commission SET state='available', reservation_token=NULL, stripe_session_id=NULL, customer_email=NULL, problem=NULL, desired_outcome=NULL, current_approach=NULL, notes=NULL, reserved_until=NULL, updated_at=CURRENT_TIMESTAMP WHERE slot=1")
    conn.commit(); conn.close()
    return {"ok":True,"commission":"001","state":"available","refunded":True,"message":"Genesis #001 was declined, the payment refund was initiated, and #001 is available again."}
